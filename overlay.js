  ...
  window.__mp = {
    ...
    think: (text) => {
      ...
      const positionBubble = () => {
        ...
        const bw = t.offsetWidth || 300;
        const bh = t.offsetHeight || 60;

        // Horizontal: center on cursor, clamp to viewport
        let left = cx - bw/2;
        left = Math.max(10, Math.min(left, window.innerWidth - bw - 10));

        // Vertical: prefer ABOVE cursor, but go BELOW if near top
        let top;
        if (cy > bh + 30) {
          top = cy - bh - 30;
        } else {
      ...
      positionBubble();
      requestAnimationFrame(() => { positionBubble(); t.classList.add('visible'); });

      // Update position when cursor moves
      const interval = setInterval(positionBubble, 100);
